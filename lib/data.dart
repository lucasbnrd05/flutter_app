...
class _DataPageState extends State<DataPage> {
  ...
  final List<String> _precipitationTypes = [
    'Flood', 'Drought', 'Fallen Trees', 'Heavy Hail', 'Heavy Rain', 'Heavy Snow', 'Other (specify in position)'
  ];
  ...
  Widget _buildInputForm() { return Form( key: _formKey, child: Column( crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[ DropdownButtonFormField<String>( value: _selectedPrecipitation, hint: const Text('Event type *'), isExpanded: true, items: _precipitationTypes.map<DropdownMenuItem<String>>((String value) => DropdownMenuItem<String>(value: value, child: Text(value))).toList(), onChanged: (String? newValue) { setState(() { _selectedPrecipitation = newValue!; }); }, validator: (value) => value == null ? 'Please select a type' : null, decoration: const InputDecoration( border: OutlineInputBorder(), ), ), const SizedBox(height: 16), TextFormField( controller: _positionController, decoration: const InputDecoration(labelText: 'Position / Description *', hintText: 'E.g., Park entrance, or Lat/Lon', border: OutlineInputBorder()), validator: (value) => (value == null || value.trim().isEmpty) ? 'Please enter a position or description' : null, maxLines: 2, ), const SizedBox(height: 10), _isLoadingLocation ? const Center(child: Padding( padding: EdgeInsets.symmetric(vertical: 8.0), child: CircularProgressIndicator(), )) : ElevatedButton.icon( icon: const Icon(Icons.my_location), label: const Text('Use Current Location'), onPressed: _getCurrentLocation, style: ElevatedButton.styleFrom(minimumSize: const Size.fromHeight(40)), ), const SizedBox(height: 20), ElevatedButton( onPressed: _saveData, style: ElevatedButton.styleFrom(minimumSize: const Size.fromHeight(50), backgroundColor: Theme.of(context).colorScheme.primary, foregroundColor: Theme.of(context).colorScheme.onPrimary), child: const Text('Save Event', style: TextStyle(fontSize: 16)), ), ], ), ); }

  Widget _buildSavedDataList() { return FutureBuilder<List<Event>>( future: _eventsFuture, builder: (context, snapshot) { if (snapshot.connectionState == ConnectionState.waiting) { return const Center(child: CircularProgressIndicator()); } else if (snapshot.hasError) { print("[DataPage _buildSavedDataList] Error: ${snapshot.error}"); return Center(child: Text("Error loading events: ${snapshot.error}", style: const TextStyle(color: Colors.red))); } else if (snapshot.hasData) { final events = snapshot.data!; if (events.isEmpty) { return const Center(child: Padding( padding: EdgeInsets.all(20.0), child: Text('No events recorded yet.'), )); } return ListView.builder( shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: events.length, itemBuilder: (context, index) { final event = events[index]; DateTime dateTimeLocal = DateTime.now(); try { dateTimeLocal = DateTime.parse(event.timestamp).toLocal(); } catch(e) { print("Error parsing timestamp: ${event.timestamp}");} final formattedDate = DateFormat.yMd().add_Hms().format(dateTimeLocal); return Card( margin: const EdgeInsets.symmetric(vertical: 4), child: ListTile( leading: Icon(_getIconForType(event.type)), title: Text(event.type, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text( '${event.description}\n' '$formattedDate', maxLines: 3, overflow: TextOverflow.ellipsis), trailing: IconButton( icon: Icon(Icons.delete_outline, color: Colors.red[300]), tooltip: 'Delete Entry', onPressed: () => _confirmDeleteEntry(event.id!), ), isThreeLine: true, dense: true, onTap: () { print("[DataPage] Navigating to detail for event ID: ${event.id}"); Navigator.push( context, MaterialPageRoute( builder: (_) => EventDetailPage(event: event), ), ); }, ), ); }, ); } else { return const Center(child: Text("Something went wrong.")); } } ); }
