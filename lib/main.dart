...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _checkAndShowWelcomeDialog() async {
    try {
      ...
      bool alreadyShown = prefs.getBool(_welcomeDialogShownKey) ?? false;

      print("[HomePage] Welcome dialog already shown: $alreadyShown");
      ...
      print("[HomePage] Error checking/showing welcome dialog: $e");
    }
  }
  ...
  Future<void> _checkApiKeyAndFetch() async {
    ...
    if (!mounted) return;
    final bool keyFound = apiKey != null && apiKey.isNotEmpty;
    if (mounted) {
      setState(() {
        _apiKeyMissing = !keyFound;
        _apiError = null;
        _articlesFuture = null;
      });
    }
    if (keyFound) {
      print("[HomePage] NYT API Key found for user, fetching articles...");
      if (mounted) {
        setState(() {
          _articlesFuture = _nytService.fetchClimateArticles().catchError((e) {
            if (!mounted) return <Article>[];
            print("[ERROR HomePage] Error caught by _articlesFuture: $e");
            if (mounted) {
              setState(() => _apiError =
              'API Error: ${e.toString().replaceFirst('Exception: ', '')}');
            }
            return <Article>[];
          });
        });
      ...
      print("[HomePage] NYT API Key missing for user, skipping fetch.");
    }
  }
          ...
          IconButton(
            ...
            onPressed: () {
              showAboutDialog(
                ...
                applicationLegalese: "© 2024 Dina Didouche",
                children: <Widget>[
                  const Padding(
                      padding: EdgeInsets.only(top: 15),
                      child: Text(
                          'An application to follow climate news and explore environmental data.'))
                ],
              );
            },
          ),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 16.0),
                  child: Text("Latest Climate News (NYT)",
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (articles.isEmpty) {
              return const Center(
                  child: Padding(
                      padding: EdgeInsets.all(30.0),
                      child: Text("No recent climate articles found.")));
            }
            return Padding(
              ...
              child: Column(
                  children: articles
                      ...
                      article: article,
                      onTap: () => _launchUrl(article.webUrl)))
                      .toList()),
            );
          } else {
            return const Center(
    ...
    if (article.imageUrl != null && article.imageUrl!.isNotEmpty) {
      ...
      return Image.network(
        ...
        errorBuilder: (context, error, stackTrace) {
          print("[ERROR ArticleCard] Error loading image: $imageUrl -> $error");
          return Container(
              height: imageHeight,
              color: placeholderColor,
              child: Center(
