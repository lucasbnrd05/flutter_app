...
import 'package:intl/intl.dart'; // Pour DateFormat (utilisé dans le popup bienvenue)
...
import 'map.dart';
import 'data.dart'; // Importe la page data
...
import 'package:flutter/foundation.dart' show kIsWeb; // Pour vérifier la plateforme
import 'dart:io'; // Seulement pour Directory

// *** FONCTION MAIN CORRIGÉE AVEC INITIALISATION HIVE ***
Future<void> main() async {
  // 1. Assurer l'initialisation Flutter (Obligatoire avant Hive/async)
  ...
  if (!kIsWeb) {
    try {
      ...
      await Hive.initFlutter(); // Fallback
      print("[WARN main] Hive initialized without specific path (fallback).");
    }
  } else {
    await Hive.initFlutter(); // Pour le web
    print('[INFO main] Hive initialized for Web.');
  }

  // 3. Ouvrir la Box Hive nécessaire pour DataPage et MapPage
  try {
    // Important: Ouvre la box AVANT runApp
    await Hive.openBox<Map>('precipitationBox');
    print("[INFO main] Hive box 'precipitationBox' opened successfully.");
  } catch (e) {
    print("[ERROR main] Error opening Hive box 'precipitationBox': $e");
    // L'application se lancera quand même, mais la condition isBoxOpen sera fausse
  }
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      darkTheme: darkTheme,
      home: const HomePage(),
      // Assure-toi que toutes les routes nécessaires sont là
      routes: {
        '/home': (context) => const HomePage(),
        '/map': (context) => const MapPage(),
        '/about': (context) => const AboutPage(),
        '/settings': (context) => const SettingsPage(),
        '/data': (context) => const DataPage(), // Route pour la page Data
      },
    );
  }
}
...
// Cette partie est la même que la version que tu avais demandée
// avec le bouton "How to Help?"
...
class _HomePageState extends State<HomePage> {
  // ... (Variables _quotes, _ecoTips, _nytService, _apiKeyMissing, _apiError inchangées) ...
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) { // Vérifie si monté avant d'afficher
        _showWelcomeDialog(context);
      }
    });
  }

  // --- Fonctions (welcome dialog, refresh, check key, launch url) ---
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar( title: const Text("GreenWatch 🌍"), actions: [ IconButton( icon: const Icon(Icons.info_outline), tooltip: 'About GreenWatch', onPressed: () { showAboutDialog( context: context, applicationName: "GreenWatch", applicationVersion: "1.0.0", applicationLegalese: "© 2024 GreenWatch", children: <Widget>[ const Padding(padding: EdgeInsets.only(top: 15), child: Text('An application to follow climate news and explore environmental data.')) ], ); }, ), IconButton( icon: const Icon(Icons.refresh), tooltip: 'Refresh articles', onPressed: _refreshData, ), ], ),
          ...
          child: Column(
            ...
            children: [
              ...
              Padding( padding: const EdgeInsets.symmetric(horizontal: 16.0), child: Wrap( spacing: 12.0, runSpacing: 8.0, alignment: WrapAlignment.center, children: [ ElevatedButton.icon( icon: const Icon(Icons.map_outlined), label: const Text("View Map"), onPressed: () => Navigator.pushNamed(context, '/map'), style: ElevatedButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10))),
                ...
                // *** FIN DU BOUTON ***
                // Le bouton "Report Data" est ajouté pour la navigation
                ElevatedButton.icon( icon: const Icon(Icons.add_chart), label: const Text("Report Data"), onPressed: () => Navigator.pushNamed(context, '/data'), style: ElevatedButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10)), ),
              ], ), ),
              const SizedBox(height: 30),
...
class ArticleCard extends StatelessWidget {
  ...
  Widget _buildImage(BuildContext context) {
    ...
    if (article.imageUrl != null && article.imageUrl!.isNotEmpty) {
      ...
      if (!imageUrl.startsWith('http://') && !imageUrl.startsWith('https://')) { imageUrl = 'https://static01.nyt.com/$imageUrl'; }
      return Image.network( imageUrl, height: imageHeight, width: double.infinity, fit: BoxFit.cover,
        ...
        errorBuilder: (context, error, stackTrace) { print("[ERROR ArticleCard] Error loading image: $imageUrl -> $error"); return Container(height: imageHeight, color: placeholderColor, child: Center(child: Icon(Icons.broken_image, color: errorIconColor, size: 40))); },
      );
